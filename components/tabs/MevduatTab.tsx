  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="bg-green-50 rounded-xl p-7 border border-green-200">
        <h3 className="font-bold text-gray-900 mb-4">En Yüksek Mevduat Faizleri — {vade} Ay</h3>
        <div className="space-y-3">
          {filtered.map(t => {
            const bank = getBankById(t.bankId)!;
            return (
              <div key={`${t.bankId}-${t.vade}`} className="bg-white rounded-lg p-3.5 flex items-center justify-between border border-gray-200">
                <div className="flex items-center gap-3">
                  <BankLogo bankId={t.bankId} size="sm" />
                  <div>
                    <div className="font-semibold text-sm">{bank.name}</div>
                    <div className="text-xs text-gray-400">{vade} ay • min. {formatCurrency(t.minTutar)}</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="font-bold text-green-700">%{t.yillikFaiz.toFixed(2)}</div>
