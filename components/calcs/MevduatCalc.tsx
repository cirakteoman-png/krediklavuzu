  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="bg-surface rounded-xl p-7 shadow-sm border border-line">
        ...
        <div className="space-y-4">
          <label className="block">
            ...
            <div className="relative">
              <input type="number" value={tutar} onChange={e => setTutar(+e.target.value)} step={10000}
                className="w-full border border-line rounded-lg px-4 py-2.5 focus:outline-none focus:border-brand pr-8" />
          ...
          <label className="block">
            ...
            <select value={vade} onChange={e => setVade(+e.target.value)}
              className="w-full border border-line rounded-lg px-4 py-2.5 focus:outline-none focus:border-brand">
        ...
        {result && (
          <div className="mt-4 bg-brand/5 border border-brand/20 rounded-lg p-4 space-y-2">
            ...
            <div className="flex justify-between border-t border-brand/20 pt-2 mt-1">
      ...
      <div className="bg-brand/5 rounded-xl p-7 border border-brand/20">
        <h3 className="font-bold text-ink mb-4">En Yüksek Mevduat — {vade} Ay</h3>
        <div className="space-y-3">
          {filtered.map((m, i) => (
            <div key={`${m.bank_id}-${i}`} className="bg-surface rounded-lg p-3.5 flex items-center justify-between border border-line">
              <div className="flex items-center gap-3">
                <BankLogo bankId={m.bank_id} size="sm" />
                <div>
                  <div className="font-semibold text-sm">{m.banks?.name}</div>
                  <div className="text-xs text-ink-soft">{vade} ay • min. {formatCurrency(m.yillik_faiz)}</div>
                </div>
              </div>
              <div className="text-right">
                <div className="font-bold text-brand">%{m.yillik_faiz.toFixed(2)}</div>
